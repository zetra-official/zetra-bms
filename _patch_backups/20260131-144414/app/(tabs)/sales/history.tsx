  ...
  const load = useCallback(
    async (mode: "boot" | "refresh") => {
      ...
      try {
        ...
        // Try common signatures safely (no DB change):
        // attempt 1: get_sales(p_store_id)
        let res = await supabase.rpc("get_sales", {
          p_store_id: activeStoreId,
        } as any);

        // attempt 2: maybe expects limit/offset
        if (res.error) {
        ...
        // attempt 3: maybe expects store_id not p_store_id
        if (res.error) {
          res = await supabase.rpc("get_sales", {
