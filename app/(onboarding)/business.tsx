...
export default function BusinessOnboardingScreen() {
  ...
  const next = useCallback(() => {
    ...
    if (!bn) {
      Alert.alert("Business name required", "Tafadhali andika jina la biashara.");
      return;
    }

    // Safer for spaces/special chars in params
    const safeName = encodeURIComponent(bn);

    router.push({
      pathname: "/(onboarding)/store",
      params: { businessName: safeName },
    });
  }, [businessName, router]);
