...
} from "@/src/ai/business/businessBridge";
import {
...
} from "@/src/ai/business/semanticIntentClient";
...
function getBusinessDateForTimezone(
...
function mapSemanticDomainToLegacyBusinessIntent(
  semantic: any
): "SALES" | "PROFIT" | "INVENTORY" | "PRODUCT" | "GENERAL" {
  const domain = upper(semantic?.domain);
  const intent = upper(semantic?.intent);
...
function mapSemanticIntentToAppRoute(
  semantic: any
):
  | "GENERAL_CHAT"
  | "BUSINESS_ANALYSIS"
...
const callWorkerChat = useCallback(
  async (
    ...
    if (!requireWorkerUrlOrAlert()) {
      throw new Error("Worker URL missing");
    }

    const localRoute = detectAppRoute(text, false);
    ...
    let detectedIntent:
      ...
      | "INVENTORY"
      | "PRODUCT"
      | "GENERAL" = detectBusinessIntent(text);

    const pureDecisionMode =
      detectPureDecisionMode(text);

    const analysisFollowupMode =
      detectAnalysisFollowupIntent(text);

    const businessCalcBypass =
      detectBusinessCalcBypass(text);

    let semanticIntent: any = null;
...
const shouldRunSemanticClassifier =
  localRoute === "GENERAL_CHAT" ||
  localRoute === "DEFINITION_EXPLANATION" ||
  localRoute === "WRITING_ASSIST" ||
  localRoute === "BUSINESS_ANALYSIS" ||
  localRoute === "BUSINESS_FORECAST" ||
  localRoute === "BUSINESS_COACH";
console.log(
  "\n========== ZETRA ROUTING DEBUG =========="
);
console.log("QUESTION:", text);
console.log("LOCAL ROUTE:", localRoute);
console.log("DETECTED INTENT:", detectedIntent);
console.log(
  "SHOULD RUN SEMANTIC:",
  shouldRunSemanticClassifier
);
console.log(
  "ACTIVE ORG:",
  clean(org.activeOrgId) || "MISSING"
);
console.log(
  "ACTIVE STORE:",
  clean(org.activeStoreId) || "MISSING"
);
console.log(
  "=========================================\n"
);
    if (shouldRunSemanticClassifier) {
      try {
        const auth =
  ...
  "===============================================\n"
        ...
        if (
          ...
          const semanticResult =
            await requestSemanticBusinessIntent({
              workerUrl:
                AI_WORKER_URL,
...
if (!semanticResult.ok) {
  ...
  console.log(
     ...
     * ========================================================================
     *
     * OpenAI/semantic layer understands the user's business question.
     * ZETRA canonical engine determines the verified business numbers.
     *
     * Source of truth:
    ...
    let canonicalBusinessResult: any = null;
    let canonicalBusinessContext = "";
    ...
    if (
      ...
      if (organizationId) {
        try {
          ...
          console.log(
            "\n========== ZETRA CANONICAL BUSINESS DEBUG =========="
          );
          ...
          if (
            canonicalBusinessResult?.status ===
            "SUCCESS"
          ) {
            if (
              canonicalBusinessResult.resultType ===
                "PERIOD" &&
              canonicalBusinessResult.periodResult
                ?.metrics
            ) {
              const metrics =
                canonicalBusinessResult
                  .periodResult.metrics;

              canonicalBusinessContext = [
                "VERIFIED ZETRA BUSINESS DATA:",
                "Source: ai_daily_store_snapshots_v1",
                "Data status: VERIFIED",
                `Requested period: ${clean(
                  metrics.fromDate
                )} to ${clean(
                  metrics.toDate
                )}`,
                `Latest snapshot date: ${clean(
                  metrics.latestSnapshotDate
                )}`,
                `Days with snapshots: ${num(
                  metrics.daysWithSnapshots
                )}`,
                `Stores represented: ${num(
                  metrics.storesCount
                )}`,
                "",
                "FINANCIAL METRICS:",
                ...
                `- Average order value: ${num(
                  metrics.averageOrderValue
                )}`,
                `- Gross margin percent: ${num(
                  metrics.grossMarginPercent
                )}`,
                `- Net margin percent: ${num(
                  metrics.netMarginPercent
                )}`,
                "",
                "DATA QUALITY:",
                `- COGS coverage percent: ${num(
                  metrics.cogsCoveragePercent
                )}`,
                `- COGS items counted: ${num(
                  metrics.cogsItemsCount
                )}`,
                `- COGS missing items: ${num(
                  metrics.cogsMissingItemsCount
                )}`,
                "",
                "INVENTORY POSITION:",
                `- Stock quantity: ${num(
                ...
                `- Reserved quantity: ${num(
                  metrics.reservedQty
                )}`,
                `- Available stock quantity: ${num(
                  metrics.availableStockQty
                )}`,
                `- Products in stock: ${num(
                  metrics.productsInStock
                )}`,
                `- Out of stock products: ${num(
                  metrics.outOfStockProducts
                )}`,
                `- Low stock products: ${num(
                  metrics.lowStockProducts
                )}`,
                "",
                "STRICT RESPONSE RULES:",
                "- These numbers are canonical verified ZETRA data.",
                "- Use these numbers when answering the user's business question.",
                "- Never replace these values with guessed or generic business claims.",
                "- Do not claim the business is doing well or badly unless supported by these metrics.",
                "- If giving analysis, clearly distinguish verified facts from recommendations.",
                "- Inventory values represent the latest available inventory position and must not be summed across dates.",
              ].join("\n");
            } else if (
              canonicalBusinessResult.resultType ===
                "COMPARISON" &&
              ...
              canonicalBusinessContext = [
                ...
                semanticIntent?.periodPreset === "THIS_WEEK" && semanticIntent?.comparisonMode === "PREVIOUS_WEEK" ? "- COMPARISON BASIS: Compare week-to-date with the equivalent elapsed days of the previous week. The previous range is NOT the entire previous week." : "",
              ...
              canonicalBusinessContext = [
                "VERIFIED ZETRA STORE COMPARISON:",
                "Source: ai_daily_store_snapshots_v1",
                "Data status: VERIFIED",
                JSON.stringify(
                  canonicalBusinessResult
                ...
                "",
                "STRICT RESPONSE RULES:",
                "- Rank stores only from the verified data above.",
                "- Do not invent store performance.",
              ].join("\n");
            } else if (
              canonicalBusinessResult.resultType ===
                "LATEST_STORE" &&
              ...
              canonicalBusinessContext = [
                "VERIFIED ZETRA BUSINESS DATA:",
                "Source: ai_daily_store_snapshots_v1",
                "Data status: VERIFIED FALLBACK",
                "IMPORTANT: The requested date had no canonical snapshot.",
                `Actual snapshot date: ${clean(
                  canonicalBusinessResult
                    .latestStoreResult
                    .snapshot
                    .snapshotDate
                ...
                "",
                "STRICT RESPONSE RULES:",
                "- Tell the user that this is the latest verified snapshot, not necessarily today's data.",
                "- Never present this snapshot as belonging to the requested date.",
                "- Do not invent missing business values.",
              ].join("\n");
            }
          } else {
            canonicalBusinessContext = [
              "ZETRA VERIFIED BUSINESS DATA STATUS:",
              `Status: ${clean(
                canonicalBusinessResult?.status
              )}`,
              `Code: ${clean(
                canonicalBusinessResult?.code
              )}`,
              `Message: ${clean(
                canonicalBusinessResult?.message
              )}`,
              "",
              "STRICT RESPONSE RULE:",
              "Verified canonical business data is unavailable for this request. Do not invent sales, profit, expenses, orders, stock, trends, or claims that the business is doing well or badly.",
            ].join("\n");
          }
        } catch (canonicalError: any) {
          console.log(
            "\n========== ZETRA CANONICAL BUSINESS ERROR =========="
          );
          ...
          canonicalBusinessContext = [
            ...
            "",
            "STRICT RESPONSE RULE:",
            "Canonical business data could not be verified. Do not invent business numbers or performance claims.",
          ].join("\n");
        }
      }
    }
    ...
    const tasksFollowupBlock =
      allowTaskContext
        ? buildTasksFollowupContextBlock(
    ...
    const semanticContextBlock =
      semanticIntent
        ...
        ? [
            ...
            `- Period: ${clean(
              semanticIntent.periodPreset
            )}`,
            `- Comparison: ${clean(
              semanticIntent.comparisonMode
            )}`,
            `- Confidence: ${Number(
    ...
    if (
      taskIntent.asksTasks ||
      taskIntent.asksSummary ||
      taskIntent.asksOverdue ||
      taskIntent.asksWhatNow
    ) {
      return {
        text:
          buildTaskFollowupReply(
            tasksFollowupSummary,
            text,
            org.activeStoreName
          ),
            ...
            num(
              ...
              ? {
                  ...
                  message: `${num(
                    tasksFollowupSummary
                      .due_today_count
                  )} task(s) zinahitaji kufuatiliwa leo.`,
                }
              : null,
          ].filter(Boolean),
