...
export default function CheckoutScreen() {
  ...
  const [paidAmount, setPaidAmount] = useState<string>(""); // optional
  ...
  const callCreateSale = useCallback(async () => {
    // We try a couple of common payload shapes (SAFE, no DB change).
    // First attempt: { p_store_id, p_items, p_note, p_payment_method, p_paid_amount }
    ...
    // attempt 2: maybe items param called p_sale_items
    ...
    // attempt 3: maybe expects jsonb called p_items_json
    res = await supabase.rpc("create_sale", {
      p_store_id: storeId,
      p_items_json: items,
      p_note: note.trim() || null,
  ...
  const confirm = useCallback(async () => {
    ...
    // Guard numeric paidAmount (optional)
    if (paidAmount.trim()) {
    ...
    try {
      ...
      // We expect create_sale to return either:
      // - sale_id (uuid) OR
      // - { sale_id: uuid } OR
      // - a row containing sale_id
  ...
  const mismatch =
    activeStoreId && storeId && activeStoreId !== storeId ? true : false;
    ...
    <Screen bottomPad={140}>
      <View style={{ flex: 1, gap: 14 }}>
        ...
        {mismatch && (
          <Card>
            <Text style={{ color: theme.colors.muted, fontWeight: "800" }}>
              Note: Active store imebadilika. Checkout inatumia store iliyoletwa
              (storeId) ili iwe stable.
            </Text>
          </Card>
        )}
